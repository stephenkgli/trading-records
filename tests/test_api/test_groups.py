...
class TestGroupsAPI:
    def test_groups_recompute_and_list(self, client, db_session, seed_group_trades):
        seed_group_trades()

        recompute_resp = client.post("/api/v1/groups/recompute")
    ...
    def test_group_detail_and_patch(self, client, db_session, seed_group_trades):
        seed_group_trades()
        client.post("/api/v1/groups/recompute")

        groups_resp = client.get("/api/v1/groups?symbol=AAPL")
        group_id = groups_resp.json()["groups"][0]["id"]

        detail_resp = client.get(f"/api/v1/groups/{group_id}")
    ...
    def test_groups_sort_and_pagination_are_global(
        self, client, db_session, make_trade_group
    ):
        make_trade_group(
            symbol="AAPL",
    ...
    def test_groups_filter_empty_asset_classes_returns_empty(
        ...
        resp = client.get("/api/v1/groups?asset_classes=")
        assert resp.status_code == 200
        data = resp.json()
